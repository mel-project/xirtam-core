...
use nullpoint_structs::event::{Event, Recipient};
use nullpoint_structs::group::GroupMessage;
use nullpoint_structs::server::{AuthToken, MailboxId, ServerClient, SignedMediumPk};
...
async fn send_group_message(
    ctx: &AnyCtx<Config>,
    group_id: nullpoint_structs::group::GroupId,
    mime: &SmolStr,
    body: &Bytes,
) -> anyhow::Result<NanoTimestamp> {
    let db = ctx.get(DATABASE);
...
) -> anyhow::Result<Vec<nullpoint_crypt::dh::DhPublic>> {
...
fn recipients_from_peer(peer: &UserInfo) -> anyhow::Result<Vec<nullpoint_crypt::dh::DhPublic>> {
