...
There are three kinds of participants in the Nullspace protocol: **clients**, **servers**, and the **directory**.
...
Nullspace is an *end-to-end encrypted* chat system. This means that generally, clients are the only place where plaintext messages exist. Servers and the directory only see encrypted payloads plus the routing metadata required to deliver them.
...
The directory is a centralized, append-only registry that is the root of trust for the entire public key infrastructure of Nullspace. It maps:
