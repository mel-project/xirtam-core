...
impl ProfileLoader {
    pub fn view(&mut self, rpc: InternalClient, username: &UserName) -> ProfileView {
        ...
        if should_fetch {
            ...
            let promise = Promise::spawn_async(async move {
                flatten_rpc(rpc.user_profile(username).await)
            });
            entry.inflight = Some(promise);
        }
    ...
    pub fn label_for(&mut self, rpc: InternalClient, username: &UserName) -> DisplayLabel {
        let view = self.view(rpc, username);
        let display = view
            .display_name
            .clone()
