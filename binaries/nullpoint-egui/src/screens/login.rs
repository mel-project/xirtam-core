...
pub struct Login<'a>(pub &'a mut NullpointApp);
...
impl Widget for Login<'_> {
    fn ui(self, ui: &mut eframe::egui::Ui) -> Response {
        ...
        let register_info = ui.use_state(|| None::<nullpoint_client::internal::RegisterStartInfo>, ());
            ...
            match *step {
                LoginStep::EnterUsername => {
                    ...
                    if register_start.is_running() {
                        ...
                        let username = match username_str.parse::<nullpoint_structs::username::UserName>() {
                ...
                LoginStep::FinishBootstrap => {
                    ...
                    ui.label(
                        RichText::new(
                            "Hint: ~public_test is the test server run by the Nullpoint developers",
                        )
                        .size(10.0),
                    );
                        ...
                        .clicked()
                    {
                        let server_name = match server_str
                            .parse::<nullpoint_structs::server::ServerName>()
                        {
                        ...
                        let request = nullpoint_client::internal::RegisterFinish::BootstrapNewUser {
                            username,
                            server_name,
                        };
                        let rpc = self.0.client.rpc();
                ...
                LoginStep::FinishAddDevice => {
                        ...
                        .clicked()
                    {
                        ...
                        let bundle = nullpoint_client::internal::NewDeviceBundle(raw.into());
                        let request = nullpoint_client::internal::RegisterFinish::AddDevice { bundle };
                        let rpc = self.0.client.rpc();
                        let promise = Promise::spawn_async(async move {
                            flatten_rpc(rpc.register_finish(request).await)
                        });
