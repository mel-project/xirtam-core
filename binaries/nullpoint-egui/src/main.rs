...
use url::Url;
use nullpoint_client::{Client, Config, internal::Event};
...
const DEFAULT_DIR_ENDPOINT: &str = "https://nullpoint-test-directory.nullfruit.net/";
const DEFAULT_DIR_ANCHOR_PK: &str = "bpOJ5ga-oQjb0njgBV5CtEZIVU6wjvltXjsQ_10BNlM";

#[derive(Debug, Parser)]
#[command(name = "nullpoint-egui", about = "Minimal nullpoint GUI client")]
struct Cli {
    #[arg(long)]
...
impl eframe::App for NullpointApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if let Ok(event) = self.recv_event.try_recv() {
            tracing::debug!(event = ?event, "processing nullpoint event");
            match event {
...
fn main() -> eframe::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(tracing_subscriber::EnvFilter::new(
            "nullpoint=debug,nullpoint_egui=debug",
        ))
        .init();
    let cli = Cli::parse();
...
fn default_db_path() -> PathBuf {
    ...
    let dir = base_dir.join("nullpoint-egui");
    if let Err(err) = std::fs::create_dir_all(&dir) {
        tracing::warn!(error = %err, "failed to create config dir");
    }
    dir.join("nullpoint-client.db")
}
